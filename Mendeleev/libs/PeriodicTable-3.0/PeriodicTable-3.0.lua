...
function getItemID(item)
	...
	return tonumber(item) or tonumber(string.match(item, "item:(%d+)")) or (-1 * (string.match(item, "enchant:(%d+)") or 0))
end
function makeNonPresentMultiSet(parentname)
	...
	local escapedparentname = string.gsub(parentname, "([%.%(%)%%%+%-%*%?%[%]%^%$])", "%%%1")
	-- Check all the sets to see if they start with this name.
	for k,v in pairs(sets) do
		if string.match(k, "^"..escapedparentname.."%.") then
...
-- These two functions are for use with string.string.gsub.  The first simply returns the number, the second
...
function uncompress(setstring)
	...
	if string.sub(setstring, 1,2) == "m," or string.sub(setstring, 1,2) == "u," then
		-- Do nothing to these, they come uncompressed.
		return setstring
	elseif string.sub(setstring, 1,2) == "o," then
		-- Uncompress the itemid but NOT the value.
		return string.gsub(setstring, "(%-?)(%w+)(:)",fromb36c)
	elseif string.sub(setstring, 1,2) == "c[" then
		-- Chop it into pieces, uncompress those, reassemble, and return.
		local newsetstring = "c"
		for set in string.gmatch(setstring, "(%b[])") do
			local childstring,rest = string.match(string.sub(set, 2,-2), "([^,]+,)(.+)$")
			newsetstring = newsetstring.."["..childstring..uncompress(rest).."]"
		end
		return newsetstring
	else
		-- Uncompress every damn thing.
		return string.gsub(setstring, "(%-?)(%w+)",fromb36)
	end
end
...
function isInCType(set)
	...
	if parentset and string.len(parentset) > 0 then
		-- Since we found a set at this parent point, let's escape matchy characters now in the child name
		child = string.gsub(child, "([%.%(%)%%%+%-%*%?%[%]%^%$])", "%%%1")
		if string.sub(parentset, 1,2) == "c[" and string.match(parentset, "%["..child..",") then
...
function shredCTypeCache(setname, setdata)
	...
	for subset in string.gmatch(setdata, "(%b[])") do
		local childname, rest = string.match(string.sub(subset, 2), "([^,]+),(.+)$")
		if string.sub(rest, 1,2) == "c[" then
...
function shredCache(setname)
	...
	local parentname = string.match(setname, "^(.+)%.[^%.]+$")
	if parentname then
...
cache = setmetatable({}, {
	...
	__index = function(self, key)
		...
		-- Grab the first two characters so we don't need to call :string.sub 5 times
		local firsttwo = string.sub(setstring, 1,2)
		if firsttwo == "m," then
			-- This table is a list of references to the members of this set.
			self[key] = {}
			local working = self[key]
			for childset in string.gmatch(string.sub(setstring, 3), "([^,]+)") do
					...
					if pointer then
						...
						if type(firstv) == "table" then
							...
							for _,v in ipairs(pointer) do
								working[getn(working)+1] = v
							end
						elseif firstv then
							-- This is not a multiset, just stick a reference in.
							working[getn(working)+1] = pointer
						end
					end
				end
			end
		...
		self[key] = {}
		local working = self[key]
		for itemstring in string.gmatch(setstring, "([^,]+)") do
			-- for each item (comma seperated)..
			-- ...check to see if we have a value set (ie "14543:1121")
			local id, value = string.match(itemstring, "^([^:]+):(.+)$")
...
function PT3:AddData(arg1, arg2, arg3)
	...
	if not arg3 and type(arg2) == "string" then
		...
		if arg3 then
			...
			local version = tonumber(string.match(arg2, "(%d+)"))
...
function activate(self, oldLib, oldDeactivate)
	local setsmeta = {
		__index = function(self, key)
			local base = string.match(key, "^([^%.]+)%.") or key
