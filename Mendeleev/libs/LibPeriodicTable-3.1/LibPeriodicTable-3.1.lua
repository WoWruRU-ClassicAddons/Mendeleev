...
function getItemID(item)
	...
	return tonumber(item) or tonumber(string.match(item, "item:(%d+)")) or (-1 * ((string.match(item, "enchant:(%d+)") or string.match(item, "spell:(%d+)")) or 0))
end
...
cache = setmetatable({}, {
	...
	__index = function(self, key)
		...
		if string.sub(setstring, 1,2) == "m," then
					...
					if pointer then
						...
						if type(firstv) == "table" then
							...
							for _,v in ipairs(pointer) do
								working[table.getn(working)+1] = v
							end
						elseif firstv then
							-- This is not a multiset, just stick a reference in.
							working[table.getn(working)+1] = pointer
						end
					end
				end
			end
...
function PT3:ItemSearch(item)
	assert(type(item) == "number" or type(item) == "string", "Invalid arg1: item must be a number or item link")
	item = tonumber(item) or tonumber(string.match(item, "item:(%d+)"))
	if item == 0 then
